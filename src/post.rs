...
impl Post {
    pub fn new(_url:&str,_acting:&str,_path:&str,_time_out:&str,_jump:bool,_data:&str,_header:&str) -> Post {
        return Post { 
            url: _url.to_string(),
    ...
    pub async fn out(&self) -> Result<HashMap<String, String>, Box<dyn Error>> {
        ...
        async fn send_acting(
            ...
            let client = reqwest::Client::builder()
                .gzip(true)
                .proxy(match reqwest::Proxy::all(data.acting.clone()) {
                    Ok(d) => d,
                    Err(e) => {
                        println!("{:?}", e);
            ...
            //判断用户是否填写了上传路径，填写了就切换到上传模式
            let body = match path {
                path if !path.is_empty() => {
                    let file = tokio::fs::File::open(data.path.to_string()).await?;
